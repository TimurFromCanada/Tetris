...
namespace Tetris
{
    ...
    {
        public const int width = 15; // Ширина поля в пикселях
        public const int height = 25; // Высота поля в пикселях
        public const int k = 15; // Размер клетки в пикселях
        ...
        public Bitmap bitfield = new Bitmap(k * (width + 1) + 1, k * (height + 3) + 1);// Создаем изображение с размерами (k*(widht)+1) и k*(height + 3) +1
        public Graphics gr; // Для рисования поля на PictureBox

        public Form1()
        {
            InitializeComponent();//Инициализируем форму
            SetShape();//Вызываем процедуру SetShape
        }

        public void FillField() //Рисуем "стакан" в котором будут распологаться объекты
        {
            var gr = Graphics.FromImage(bitfield);
            gr.Clear(Color.Aqua); //Заливаем черным цветом
        ...
        private void Form1_KeyDown(object sender, KeyEventArgs e) // Двигаем фигуру
        {
            switch (e.KeyCode)
            {
        ...
        public void SetShape() //Процедура вызова фигур
        {
            Random rnd = new Random(DateTime.Now.Millisecond); //Задаем x, по которой выбирается одна из 6 фигур
            switch (rnd.Next(7))
            {
        ...
        //timer.Start();
        //private void TimerElapsed(object sender, System.Timers.ElapsedEventArgs elapsed)
        //{
        ...
        public new void Move(int x, int y)
        {
            for (int i = 0; i < 4; i++)
            {
                shape[1, i] += x; //Смещаем координату фигуры
                shape[0, i] += y; //Смещаем координату фигуры
            }
            if (FindMistake())
            {
                Move(-x, -y);
